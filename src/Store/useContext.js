...
const UserContextProvider = ({ children }) => {
  ...
  const [imagenes, setImagenes] = useState(
    Cookies.get("imagenes") ? JSON.parse(Cookies.get("imagenes")) : []
  );
  const [loading, setLoading] = useState(false);

  const [imgsSlider, setImgsSlider] = useState(
    Cookies.get("imgsSlider") ? JSON.parse(Cookies.get("imgsSlider")) : []
  );
  ...
  useEffect(() => {
    ...
    Cookies.set("skillImg", JSON.stringify(imagenes), { expires: 365 });
    Cookies.set("carousel", JSON.stringify(imgsSlider), { expires: 365 });
